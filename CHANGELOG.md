...
- **Gitleaks secret gate verified blocking-ready**: Confirmed the working tree scans clean with the production config (`gitleaks detect --config .gitleaks.toml --no-git`: no leaks found), so flipping the CI gitleaks step to a hard blocking gate is now reduced to a one-line change — removing `continue-on-error: true` from the `ci.yml` gitleaks step. That workflow edit must be applied by a maintainer with `workflows` permission (automation agents cannot modify `.github/workflows/`). Updated `STATUS.md` and the master audit report to record the gitleaks gate as blocking-ready with the pending one-line flip tracked in the gap register and open-work list. Refreshed the full-suite validation snapshot to 1057 passed / 4 skipped and dropped stale `py -3.13` invocation prefixes from the recorded commands.
