...
- **Pages deployment and NekoBox subscription repair**: Aligned Pages with the pipeline's optional-signing mode while preserving fail-closed verification for signed artifacts, added exact-404 first-deployment bootstrap when no rollback manifest exists, and introduced dedicated `nekobox*.json` top-level outbound arrays so NekoBox can import separate nodes without misusing complete sing-box/Xray profile documents. Updated validators, output contracts, frontend client routing, localized descriptions, compatibility docs, and regression coverage.
...
- **Scanner and frontend behavior**: Standalone Go scanning requires explicit opt-in, bounds IPv4 target expansion, rejects unsupported IPv6 ranges, and drains results without abandoning the producer. Laboratory loads retry after transient failure; fuzzy search retains relevance without a redundant latency sort. Removed the unused health-score renderer. Go verifier preflight now matches the tester module's Go 1.26 minimum.

- **Scheduled release and verification repairs**: The metadata schema accepts the usable-source and transport-success counters emitted by shard aggregation. An aggregation/reconciliation regression covers the public schema boundary. Local Go verification now uses the same legacy-tester linker compatibility flag as CI, without applying it to the uTLS module. Removed the unreferenced frontend target-roadmap report with retired implementation blueprints.
- **Tester startup and shutdown budgets**: Python fallback config and wrapper readiness now share an explicit loopback HTTP port. Go chains reuse an existing direct outbound. An open revival circuit breaker retains unverified candidates without restarting Python recovery for every batch; consumers stop starting optional revival after intake stops. Source-quality failures retain sanitized diagnostics, while Python startup failures no longer log raw tracebacks.
- **Frontend runtime cleanup**: Completed the inline SVG migration and removed Feather's duplicate runtime. Removed the nonfunctional BYOW generator controls, their obsolete translations and styling, and the retired health-score stylesheet; the remaining Worker card now describes the required manual deployment accurately. The proxy-page CSP permits WebAssembly compilation while continuing to reject JavaScript eval. Regression checks cover actual Go loopback connectivity instead of duplicate parser or source-string assertions. GeoIP lookup tests mock the database-opening boundary without depending on local MMDB files.
- **Residual tester lifecycle hardening**: Release Python fallback port leases when configuration serialization fails, and suppress malformed Go honeypot probes when secure handshake-packet construction cannot produce the required payload.

- **Previously omitted browser coverage**: The browser profile now includes Laboratory interaction and rendered screenshot checks, shares explicit executable browser validation across modules, and fails required checks on browser crashes. Laboratory tests use the served site so ES modules execute. Invalid or empty canonical chains cannot silently restore stale legacy paths, disabled DNS output modes emit empty required artifacts without rebuilding datasets, explicit zero telemetry survives dictionary-based metadata generation, and source drop statistics stay attached until the first successful chunk enqueue.
...
- **Concurrent file I/O resilience**: Async writes now use the atomic writer in a worker thread, JSON serving retries transient sharing failures, file locks close handles on acquisition failure and use a stable lock byte, and file-operation diagnostics sanitize sensitive values.
- **Cache input validation**: Reject non-finite and far-future test timestamps and normalize overflowing counters so malformed cache entries cannot crash cache reads or remain fresh indefinitely.
- **Public output compatibility and PR review closure**: Classified API proxy arrays separately from native client documents, added a whole-document proxy-list schema, extended validation to every root/country/protocol/chosen/chain config, and enforced alias parity and path containment. Corrected sing-box evasion output to use flattened official dial fields and multiplex-only padding, removed unsupported TLS padding/fragmentation structures, hardened current Xray transport validation, preserved unverified shielded candidates without inflating working counts, reconciled quarantine-aware shard health, fixed managed Vwarp availability, and addressed frontend sort/cache and SVG validation review findings. Generated fixtures pass native sing-box, Xray, and Mihomo validation.
- **Final independent review and consolidation**: Re-ran the remediation diff as an untrusted change, removed stale audit snapshots and orphaned developer tooling, consolidated source/dependency/workflow ownership, eliminated vendored font binaries, tightened DNS/source-admission/browser/rollback trust contracts, and expanded the unified verifier. Release posture remains conditional until exact-head CI, supported toolchains, sealed artifacts, history review, and live deployment evidence are available.
- **Final source and verifier hardening**: Canonicalized 1,028 fetch locators by removing client-only fragments/default ports, rejected credential-bearing or invalid source URLs, eliminated duplicate network work with minimal shard movement, removed tracked runtime cache/database placeholders, and hardened verification subprocess isolation, timeout recovery, and composable static/release-tail evidence.
...
- **Full-Scope PR #526 Audit Remediation**:
  ...
  - **Topology artifact cleanup**: Removed the generated interactive topology artifact and its one-off generator from the active source tree; module ownership and import-cycle contracts now provide deterministic structural evidence.
...
- **Consumer decomposition, sing-box schema validation, and security transport hardening**:
  ...
  - Explicitly configured Pydantic's `ConfigDict` to ignore extra fields (`extra="ignore"`) on the core `Proxy` model in `models.py`, preventing untrusted or malicious extra keys in input configurations from propagating to downstream serialization.
...
- **CSP script hardening, configurable notification, and startup warning**:
  ...
  - Added a loopback-aware security warning on server startup if admin authentication is bypassed in non-production environments while bound to a non-loopback host interface.
  - Excluded `scripts/validate_test_skips.py` and `tests/unit/test_validate_test_skips.py` from debt scanning to resolve false-positive `TODO` markers.

- **Whole-system telemetry, cache, parser, and security hardening**: Promoted `pipeline_events.jsonl` into the validated Pages artifact contract with sanitized JSONL event records, manifest inclusion, output-matrix documentation, deploy-smoke fixtures, and validator coverage. Added EventStream JSONL persistence, cross-platform locked test-cache writes, OpenVPN multi-remote recovery, stricter proxy address validation, safer log userinfo masking, VLESS Worker UDP frame buffering, frontend IndexedDB snapshot-version probing, and explicit event-listener wiring for project-owned UI callbacks.
- **Workflow action integrity and Pages telemetry fallback**: Resolved every external action tag to its repository commit, pinned all `uses:` references to immutable commit SHAs with readable version comments, and made `main.yml`/`deploy-pages.yml` initialize a sanitized `pipeline_events.jsonl` fallback before refreshing the Pages artifact manifest. `scripts/validate_action_pins.py` verifies the checked-in resolution manifest and `scripts/validate_workflows.py` requires telemetry initialization during public artifact preparation.
- **Source-of-truth consolidation and stale-doc cleanup**: Moved current release truth to `docs/readiness.json`, generated `STATUS.md`, the changelog, and machine-readable governance matrices; removed the parallel root master report and point-in-time audit/roadmap snapshots from the active tree. Documentation hygiene now rejects stale review ledgers and duplicate truth surfaces.
...
- **Pipeline Timeout Shard Robustness and CI Remediation**: 
  ...
  - Patched `deploy_mirror.yml` and `retest.yml` workflows to gracefully bypass rather than fail the job when `gh run download` cannot find the `pipeline-output` artifact.
  - Upgraded `aiohttp` to `3.14.0` in `requirements.txt` and `requirements-prod.txt` to resolve `pip-audit` vulnerability alert.

- **Code-quality audit remediation (f1-f12 + frontend review)**: Closed a full non-security code-quality audit pass across the Python backend, the Go uTLS sidecar, and the frontend JavaScript surfaces. Backend fixes: corrected the Loon adapter relay format so `relay_line` defaults to `None` instead of falling back to the Surge form (`src/configstream/adapters_base.py`); implemented a real, fail-closed uTLS binary checksum verification using an `UTLS_CLIENT_SHA256` env pin or a `.sha256` sidecar with SHA-256 recomputation (`src/configstream/security/utls_wrapper.py`); removed orphaned `fetch_single_source` dead code and its now-unused imports while preserving the shared result models used by `fetcher.py` and tests (`src/configstream/fetcher_worker.py`); replaced the magic permanent-failure threshold with a named `PERMANENT_FAILURE_SENTINEL` constant and an `_is_permanent_failure()` helper (`src/configstream/source_quality.py`); wrapped the WARP scanner subprocess `communicate()` in a bounded `asyncio.wait_for` deadline that kills and returns `[]` on timeout (`src/configstream/warp_scanner.py`); fixed Vwarp version comparison to parse numeric version tuples instead of comparing version strings lexically and hardened fire-and-forget stream tasks with strong references plus `add_done_callback` GC (RUF006) (`src/configstream/tools/vwarp.py`); added a non-mutating `Proxy` membership check `TestCache.contains()` and switched cache warming to it so warm-up no longer mutates TTL state (`src/configstream/test_cache.py`, `src/configstream/cache_warming.py`); routed `adaptive_timeout` and `stego` persistence through `AtomicFileWriter` for crash-safe temp+fsync+`os.replace` writes (`src/configstream/adaptive_timeout.py`, `src/configstream/stego.py`); repaired UTF-8-as-cp1252 mojibake in comments and log strings (`intelligence/washer/core.py`, `output_logic.py`, `output_handler.py`, `generators/clash.py`). Operability/auth hardening: added optional per-user Telegram bot authorization gated by a new `TELEGRAM_ALLOWED_USERS` setting with a `require_authorization` decorator on privileged handlers and startup warnings when unset (`src/configstream/config.py`, `src/configstream/bot_cli.py`, with coverage in `tests/unit/test_bot_cli.py`), and made the trace-id `LogRecordFactory` install idempotently from `setup_logging` instead of at import time (`src/configstream/logging_config.py`). Go fix: corrected a mangled `InsecureSkipVerify` source comment (`src/go/utls_client/main.go`). Frontend fixes: removed a dead verification branch and wired `runLocalVerification` to fetch `artifact_manifest.json` and verify its Ed25519 signature via the Web Crypto path (`frontend/assets/js/verifier.js`), and bounded the Chart.js load-poll `setInterval` so it gives up after 10s instead of polling forever (`frontend/assets/js/charts.js`). The larger frontend UI scripts (`i18n.js`, `lab.js`, `analytics.js`, `statistics.js`, `utils/dom.js`) and the auxiliary `scripts/` uploaders/validators were reviewed by triage; the only behavioral fix needed was the bounded chart poll, while `scripts/upload_telegram.py` and `scripts/upload_hf.py` were flagged for embedding tokens in URLs/remotes where a broad `except` could surface them in CI logs (tracked, no token rotation required). All ruff, mypy, and full pytest gates remained green after the changes.
- **Gitleaks secret gate made blocking**: Confirmed the working tree scans clean with the production config and removed stale status/master language that treated gitleaks as a future maintainer action. CI now records gitleaks/source-token scanning as a hard blocking gate alongside Bandit, Bandit suppression hygiene, and pytest skip governance.
...
- **Source-list ownership cleanup**: Updated `scripts/deduplicate_sources.py` from the stale 14-batch layout to 17 canonical shards and removed the redundant root-level consolidated mirror.
...
- **Roadmap implementation-gate wording cleanup**: Replaced stale blockade/prohibition wording in the Part 2 roadmap, core compatibility report, capability registry, claim ledger, README scanner contract, validator, and tests. Planned Xray pipeline artifacts are now tracked as outputs requiring implementation rather than forbidden outputs; validation still prevents `docs/output_matrix.json` overclaims until generator, validator, tests, docs, and native-check semantics exist.
- **CI failure remediation batch**: Patched the reported dependency audit findings by bumping `python-dotenv` to `1.2.2` and `urllib3` to `2.7.0` in both requirement files. Restored `npm run build` as a documented alias for `build:sanity` and kept the self-contained `lab-offline.html` raw-static only so Vite sanity builds pass without changing Pages deployment inputs. Removed invalid `secrets.*` step `if:` expressions from `deploy_mirror.yml` and added a workflow validator regression to keep optional mirror deployment YAML parse-safe.
- **CI remediation verification**: Re-ran `npm run build`, same-origin frontend smoke, dependency drift, workflow/core/capability/claim/status validators, focused workflow/core/dependency/status tests, and the full pytest suite. Full local pytest now reports 1036 passed / 1 skipped. `pip-audit --no-deps` reports no known vulnerabilities for direct production pins; the dependency-resolving audit remains delegated to the CI Python 3.10-3.12 matrix because this local Python 3.13 shell cannot resolve all production wheels.
- **Python 3.10 production audit repair**: Re-pinned `numpy`, `scipy`, and `scikit-learn` to releases that resolve under CI Python 3.10 while keeping the production audit fully pinned. Verified with a Linux cp310 dry-run resolver check for `requirements-prod.txt`.
...
### CI Parity & Workflow Fixes (2026-05-13)
- **CI Fix**: Removed unused, duplicate `frontend-dist` directory and added it to `.gitignore` to enforce the canonical `frontend/` deployment policy.
- **Workflow Validation**: Fixed `scripts/validate_workflows.py` to correctly enforce the 30-day artifact retention policy on `retest.yml` and `deploy-pages.yml` in addition to `main.yml`.
- **Test Fix**: Updated unit tests in `test_validate_workflows.py` to verify the new retention checks.

### Documentation: Unified Source of Truth (2026-05-12)
- **Historical audit consolidation**: Absorbed durable status and remediation facts into `STATUS.md`, `CHANGELOG.md`, and canonical machine-readable matrices, then removed parallel point-in-time truth files.
...
- **BYOW Feature**: Users can deploy their own Cloudflare Workers for quota isolation and user-controlled bridge diversity
  - One-click deploy via Cloudflare Deploy Button
  - Frontend injection logic to personalize Gold configs with user's Worker URL
  - "Hydra Strategy" - many user-operated Worker domains reduce dependence on one shared endpoint
- **Worker Enhancements**: Updated `tools/worker.js` with private-bridge routing
