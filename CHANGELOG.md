...
- **Frontend runtime cleanup**: Completed the inline SVG migration and removed Feather's duplicate runtime. Both BYOW panels expose manual setup consistently after localization. The proxy-page CSP permits WebAssembly compilation while continuing to reject JavaScript eval. Regression checks cover actual Go loopback connectivity and browser BYOW behavior instead of duplicate parser or source-string assertions. GeoIP lookup tests mock the database-opening boundary without depending on local MMDB files.
