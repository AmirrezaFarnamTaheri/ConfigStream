...
- **Workflow action resolution and Pages telemetry fallback**: Replaced unresolvable GitHub Actions SHA refs for setup/cache/deploy/docker/gitleaks actions with resolvable current major tags, upgraded official action refs away from Node 20-era majors where available, and made `main.yml`/`deploy-pages.yml` initialize a sanitized `pipeline_events.jsonl` fallback before refreshing the Pages artifact manifest. `scripts/validate_workflows.py` now rejects the known-bad action refs and requires telemetry initialization during public artifact preparation.
- **Source-of-truth archive absorption and stale-doc cleanup**: Restored and refreshed the root master source-of-truth report, absorbed durable policy from archived audits/roadmaps/release-hardening documents into active master/status contracts, moved machine-readable proof links off archived ledgers, and tightened documentation hygiene tests so stale overclaims and archive-as-current-source regressions fail. Cleaned BYOW trust language, frontend no-raw-HTML guidance, localized proxy-page candidate wording, offline Lab inline handlers/raw HTML insertion, and current DevOps/Security documentation.
