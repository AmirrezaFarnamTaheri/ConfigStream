...
def main() -> int:
    print(
        "WARNING: This script is deprecated. Security filtering is now handled by the pipeline stages.",
        file=sys.stderr,
    )
    args = parse_args()
    ...
    phrase = args.phrase
    filtered = [
        entry for entry in data if phrase not in (entry.get("security_issues") or [])
    ]
