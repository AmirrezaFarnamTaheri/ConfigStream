...
def validate(root: Path, native_report: Path, min_coverage: float) -> list[str]:
    ...
    if health.get("native_report_sha256") and health["native_report_sha256"] != native_digest:
        errors.append("health native report digest does not match gate input")

    xray = load(root / "xray.json")
    outbounds = xray.get("outbounds") if isinstance(xray, dict) else []
    if not any(isinstance(item, dict) and item.get("tag") not in {"direct", "block"} for item in outbounds):
    ...
    for target in ("sing-box", "xray", "mihomo"):
        if not isinstance(targets, dict) or not isinstance(targets.get(target), dict) or targets[target].get("status") not in {"generated", "passed"}:
...
def promote(root: Path, native_report: Path) -> None:
    ...
    if isinstance(files, list):
        ...
        manifest["total_size_bytes"] = sum(int(item.get("size_bytes") or 0) for item in files if isinstance(item, dict))
