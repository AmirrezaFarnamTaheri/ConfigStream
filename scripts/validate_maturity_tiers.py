...
def validate(root: Path) -> list[str]:
    ...
    try:
        data = json.loads(
            (root / "docs/maturity_tiers.json").read_text(encoding="utf-8")
        )
    ...
    for index, item in enumerate(surfaces):
        ...
        if item.get("required_release") is True and tier != "stable":
            errors.append(
                f"{surface_id}: only stable surfaces may be required for release"
            )
        ...
        if tier == "experimental" and not item.get("limitations"):
            errors.append(
                f"{surface_id}: experimental surface requires explicit limitations"
            )
