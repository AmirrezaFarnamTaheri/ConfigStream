...
def _markdown(payload: ForensicsReport) -> str:
    ...
    if scan['findings']:
        ...
        for item in scan['findings']:
            lines.append(f"| `{item['path']}` | {item['kind']} | {item['match_count']} |")
    else:
    ...
    for path, content in targets.items():
        if check:
            try:
                ...
                errors.append(f'missing repository forensics evidence: {path.relative_to(root)}')
                continue
            if current != content:
                errors.append(f'repository forensics evidence is stale: {path.relative_to(root)}')
        else:
...
def main() -> int:
    ...
    if errors:
        ...
        for error in errors:
            print(f'  - {error}')
        return 1
    print('OK: current-tree forensics is clean; upstream-history limitations are explicit')
    return 0
