...
match = re.search(r"const translations = (\{.*?\});\s*(?=function|const|let|var|$)", content, re.DOTALL | re.MULTILINE)
if not match:
...
if match:
    ...
    for m in lang_matches:
        ...
        if end_idx != -1:
            ...
            # Quote keys
            dict_str = re.sub(r'^\s+([a-zA-Z0-9._]+):', r'  "\1":', dict_str, flags=re.MULTILINE)
            # Fix trailing commas
            dict_str = re.sub(r',\s*([\]\}])', r'\1', dict_str)
            
            try:
                json_data = json.loads(dict_str)
                with open(f"{output_dir}/{lang_code}.json", "w", encoding="utf-8") as out:
