...
def inject_frontend_keys(root: Path, env: Mapping[str, str]) -> list[str]:
    ...
    if any((env.get(name) or "").strip() for name in ("STEGO_KEY", "CONFIG_STREAM_KEY")):
...
def validate_frontend_placeholders(root: Path, *, strict: bool = False) -> list[str]:
    ...
    if not constants_path.exists():
        ...
        if any(marker in constants for marker in PUBLIC_KEY_PLACEHOLDER_MARKERS):
            errors.append("Frontend PUBLIC_KEY placeholder remains in assets/js/constants.js")
    ...
    if strict:
        if not runtime_config_path.exists():
            ...
            if any(marker in runtime_config for marker in PUBLIC_KEY_PLACEHOLDER_MARKERS):
                errors.append("Frontend PUBLIC_KEY placeholder remains in assets/js/runtime-config.js")
            if STEGO_KEY_PLACEHOLDER in runtime_config:
                errors.append("Frontend STEGO_KEY placeholder remains in assets/js/runtime-config.js")
            if re.search(r"\b(?:STEGO_KEY|CONFIG_STREAM_KEY)\s*:", runtime_config):
                errors.append("Frontend runtime config must not contain a symmetric key field")
