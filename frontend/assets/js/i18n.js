const translations = {
    en: {
        ...
        "info.how.text": "Our automated engine scans 668+ public sources every 6 hours. It filters out dead nodes, verifies security (TLS, Honeypots), and optimizes routing using a hybrid Go+Python core.",
    ...
    zh: {
        ...
        "info.how.title": "工作原理",
        "info.how.text": "我们的自动化引擎每 6 小时扫描 668+ 个公开来源，自动过滤失效节点，验证安全性（TLS、蜜罐检测），并通过 Go+Python 混合内核优化路由。",
    ...
    fa: {
        ...
        "info.how.text": "موتور خودکار ما هر ۶ ساعت یک‌بار، ۶۶۸+ منبع عمومی را اسکن می‌کند. سرورهای از کار افتاده را فیلتر می‌کند، امنیت را بررسی می‌کند (TLS، تشخیص هانی‌پات) و با استفاده از هسته ترکیبی Go+Python مسیریابی را بهینه‌سازی می‌کند.",
    ...
    ru: {
        ...
        "info.how.text": "Наш автоматизированный движок сканирует 668+ публичных источников каждые 6 часов. Он фильтрует мёртвые ноды, проверяет безопасность (TLS, обнаружение honeypot) и оптимизирует маршрутизацию с помощью гибридного ядра Go+Python.",
    ...
    ar: {
        ...
        "info.how.text": "محركنا الآلي يفحص 668+ مصدر عام كل 6 ساعات. يقوم بفلترة الخوادم المعطلة، والتحقق من الأمان (TLS، كشف مصائد الجذب)، وتحسين التوجيه باستخدام نواة Go+Python المختلطة.",
