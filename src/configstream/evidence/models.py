    ...
    def _validate_contract(self) -> "ValidationEvidence":
        ...
        if self.outcome is ValidationOutcome.PASSED:
            ...
            if self.tls_attempted and (
                ...
                raise ValueError("passed TLS evidence must prove chain, hostname and no interception")
            if self.content_integrity_valid is False:
                raise ValueError("passed evidence cannot contain failed content integrity")
            if self.critical_reputation_flags:
                raise ValueError("passed evidence cannot contain critical reputation flags")
