...
def save_json(proxies: List[Proxy], path: Path, compress: bool = False) -> None:
    ...
    if compress:
        ...
        try:
            # AtomicFileWriter.write_text uses mkstemp which is fine, but for gzip we need to write compressed bytes
            # Or we can adapt write_text logic but using gzip.open.
            # Since AtomicFileWriter supports write_bytes, let's compress in memory or use a custom logic here.
            # To reuse the robust pattern:
