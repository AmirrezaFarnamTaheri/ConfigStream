                ...
                with tracker.phase("geo"):
                    for proxy in batch:
                        ...
                        if not proxy.country_code and proxy.remarks:
                            inferred_geo = infer_country_from_remarks(proxy.remarks)
                            if inferred_geo:
                                proxy.country_code = inferred_geo["country_code"]
                                proxy.country = inferred_geo["country"]
                                geo_remark_count += 1
                ...
                if chunk_source == "fetched":
                    for source, proxies_from_source in proxies_by_source.items():
                        tested_proxies_for_source = [
                            ...
                            if any(
                                proxy_unique_key(p) == proxy_unique_key(orig)
                                for orig in proxies_from_source
                            )
                        ]
