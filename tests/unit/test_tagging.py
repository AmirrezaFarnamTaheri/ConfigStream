...
def test_proxy_tagger():
    ...
    p = create_test_proxy(source="s", address="1.1.1.1", port=443, protocol="vmess", country_code="US", is_working=True)

    pt.apply([p])
    assert p.remarks == "[US] vmess"

def test_format_proxy_name_missing_data():
    p = create_test_proxy(source="s", address="1.1.1.1", port=443, protocol="vmess", is_working=True) # No country
...
def test_format_proxy_name_full():
    p = create_test_proxy(source="s", address="1.1.1.1", port=443, protocol="vmess", country_code="US", latency=50, is_working=True)
