...
def test_repository_forensics_does_not_claim_upstream_history() -> None:
    payload = json.loads(Path('docs/generated/repository-forensics.json').read_text(encoding='utf-8'))
    assert payload['source']['source_kind'] == 'archive-snapshot'
    assert payload['remediation_checkout']['history_origin'].startswith('local baseline')
