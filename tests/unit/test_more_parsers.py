...
def test_parse_v2ray_json_with_no_address():
    ...
    config = '{"outbound": {"protocol": "vmess", "settings": {"vnext": [{"port": 443, "users": [{"id": "03d011f0-38e8-4f99-9bf9-501d3c7e1f91"}]}]}}}'
    assert _parse_v2ray_json(config) is None


def test_parse_v2ray_json_with_no_port():
    """Test that _parse_v2ray_json handles JSON with no port."""
    config = '{"outbound": {"protocol": "vmess", "settings": {"vnext": [{"address": "ju-tt.name", "users": [{"id": "03d011f0-38e8-4f99-9bf9-501d3c7e1f91"}]}]}}}'
    assert _parse_v2ray_json(config) is None
...
def test_parse_tuic_with_invalid_port():
    ...
    config = "tuic://03d011f0-38e8-4f99-9bf9-501d3c7e1f91:123456@ju-tt.name:70000?congestion_control=bbr#test"
