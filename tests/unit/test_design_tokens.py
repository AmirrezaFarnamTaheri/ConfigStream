...
def test_design_tokens_present() -> None:
    ...
    for token_name, token_val in expected_tokens:
        ...
        assert re.search(pattern, css_content) is not None, (
...
def test_sticky_header_scroll_margin_offset() -> None:
    ...
    assert "scroll-margin-top" in css_content, "Missing scroll-margin-top declaration in style.css"
    ...
    assert scroll_margin_match is not None, (
...
def test_focus_visible_high_contrast_outline() -> None:
    ...
    assert re.search(
        ...
        css_content,
    ) is not None, "Expected :focus-visible outline: 2px solid var(--border-focus) or #06b6d4"

    # Verify outline-offset
    assert re.search(
        r"outline-offset\s*:\s*2px",
        css_content,
    ) is not None, "Expected outline-offset: 2px for focus indicators"
