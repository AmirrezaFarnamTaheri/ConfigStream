...
def test_validate_workflows_requires_ci_test_skip_guard(
    tmp_path: Path, monkeypatch
) -> None:
    workflow_dir = tmp_path / ".github" / "workflows"
    workflow_dir.mkdir(parents=True)
...
jobs:
  test:
    ...
    steps:
      - run: bandit -r src/configstream scripts tools frontend/assets/js -q
      - run: python scripts/validate_bandit_suppressions.py --require-active
      - run: |
          python scripts/validate_capability_registry.py
          python scripts/validate_core_compatibility.py
          python scripts/validate_module_ownership.py
...
def test_validate_workflows_requires_main_public_artifact_preparation(
    tmp_path: Path, monkeypatch
) -> None:
    workflow_dir = tmp_path / ".github" / "workflows"
    workflow_dir.mkdir(parents=True)
