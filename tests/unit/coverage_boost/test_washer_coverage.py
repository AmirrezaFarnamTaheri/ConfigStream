...
def test_washer_get_clean_endpoint(washer):
    ...
    assert ep1 == ep2
    assert ep1 in washer.clean_ips or ep1 in ["162.159.192.1", "162.159.193.10", "162.159.195.5"]

def test_washer_wash_batch(washer):
    p1 = Proxy(config="vmess://1", protocol="vmess", uuid="u1", address="1.1.1.1", port=443, details={"uuid": "u1"}, is_working=True)
    p1.country_code = "US"
...
def test_washer_wash_batch_no_working(washer):
    p1 = Proxy(config="vmess://1", protocol="vmess", uuid="u1", address="1.1.1.1", port=443, details={"uuid": "u1"}, is_working=False)
    outbounds, ids = washer.wash_batch([p1])
