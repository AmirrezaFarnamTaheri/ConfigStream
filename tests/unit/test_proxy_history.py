...
def test_history_tracker_add_update():
    ...
    assert tracker.get_proxy_history(p.id) is not None  # Changed from get_history

    stats = tracker.get_summary_stats(p.id)  # Changed from get_stats
...
def test_history_persistence(tmp_path):
    ...
    # Should load
    assert tracker2.get_summary_stats(p.id)["total_tests"] > 0
