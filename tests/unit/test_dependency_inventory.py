...
def test_dependency_inventory_covers_every_manifest_family() -> None:
    payload = json.loads(Path('docs/generated/dependency-inventory.json').read_text(encoding='utf-8'))
    assert {'python', 'npm', 'go-tester', 'go-utls', 'cargo', 'container'} == set(payload['ecosystems'])
    observed = {(item['ecosystem'], item['directory']) for item in payload['dependabot_coverage']}
