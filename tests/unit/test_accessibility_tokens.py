...
def test_tabular_numerics_css_tokens() -> None:
    ...
    assert "font-variant-numeric: tabular-nums" in css_content or "font-variant-numeric:tabular-nums" in css_content, (
        "Missing 'font-variant-numeric: tabular-nums' in style.css"
    )
    assert 'font-feature-settings: "tnum" 1' in css_content or 'font-feature-settings:"tnum" 1' in css_content or 'font-feature-settings: "tnum"' in css_content, (
        "Missing 'font-feature-settings: \"tnum\" 1' in style.css"
    )

    # Check key selectors for tabular numerics
    for selector in [".tabular-nums", ".ping-badge", ".port-cell", ".counter-value", ".stat-value"]:
...
def test_touch_target_dimensions_wcag22() -> None:
    ...
    assert re.search(touch_target_pattern, css_content, re.DOTALL) is not None, (
    ...
    assert re.search(r"--touch-target-desktop\s*:\s*24px", css_content) or re.search(
        r"--touch-target-min\s*:\s*44px", css_content
    ) or re.search(r"min-height:\s*24px", css_content) or re.search(r"min-height:\s*44px", css_content), (
