...
def test_create_chain():
    relay = Proxy(
        config="vless://1", protocol="vless", address="1.1.1.1", port=443, uuid="u1", country="TR"
    )
    exit_node = Proxy(
        config="vmess://2", protocol="vmess", address="2.2.2.2", port=443, uuid="u2", country="US"
    )

    # Mock converters
    with patch("configstream.intelligence.chaining.to_singbox_outbound", side_effect=lambda x: {"type": x.protocol}):
...
def test_generate_smart_chains():
    # Setup proxies
    relays = [
        Proxy(config="p1", protocol="hysteria2", address="1.1.1.1", port=443, uuid="u1", country="IR", country_code="IR", is_working=True),
    ]
    exits = [
        Proxy(config="p2", protocol="vmess", address="2.2.2.2", port=443, uuid="u2", country="US", country_code="US", is_working=True),
    ]
