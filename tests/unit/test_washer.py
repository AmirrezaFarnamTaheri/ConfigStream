...
def test_deterministic_warp_keys():
    ...
    fake_keys = '[{"private_key": "key1", "id": "01"}, {"private_key": "key2", "id": "02"}]'
...
async def test_smart_chain_generation():
    ...
    proxies = [
        Proxy(
            source="test", address="1.1.1.1", port=443, protocol="vless", country_code="IR",
            config="vless://...", is_working=True, tags=[]
        ),
        Proxy(
            source="test", address="2.2.2.2", port=443, protocol="vless", country_code="US",
            config="vless://...", is_working=True, tags=[]
        ),
    ]
