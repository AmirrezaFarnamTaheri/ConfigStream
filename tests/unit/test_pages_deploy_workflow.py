...
def test_retest_noop_is_qualified_before_pages_deploy() -> None:
    ...
    assert "  candidate:\n" in workflow
    assert 'source_name=$(gh api "repos/${REPOSITORY}/actions/runs/${selected}"' in workflow
...
def test_non_retest_missing_artifact_still_fails_closed() -> None:
    ...
    assert "Approved deployment source $selected did not publish pipeline-output" in workflow
