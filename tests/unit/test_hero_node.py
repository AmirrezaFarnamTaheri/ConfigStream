...
def test_hero_node_html_integration() -> None:
    ...
    assert (
        container is not None or canvas is not None
    ...
    assert (
        len(hero_script) == 1
    ), "Expected exactly one script tag referencing hero-scene.js"
    assert hero_script[0].has_attr(
        "defer"
    ), "hero-scene.js script tag must have 'defer' attribute"
...
def test_hero_scene_script_load_order_after_trust_bootstrap() -> None:
    ...
    html = INDEX_HTML.read_text(encoding="utf-8")
    scripts = re.findall(
        r'<script\b[^>]*\bsrc=["\']([^"\']+)["\']', html, flags=re.IGNORECASE
    )
...
def test_hero_scene_dpr_clamping_to_maximum_1_5() -> None:
    ...
    assert (
        "devicePixelRatio" in content
    ), "window.devicePixelRatio check not found in hero-scene.js"
    ...
    assert (
        dpr_pattern is not None
    ), "Explicit DPR clamp formula 'Math.min(window.devicePixelRatio || 1, 1.5)' required"
...
def test_hero_scene_intersection_observer_lifecycle() -> None:
    ...
    ), "IntersectionObserver not found in hero-scene.js"
    assert (
        "isIntersecting" in content
    ), "isIntersecting visibility handling not found in hero-scene.js"
...
def test_hero_scene_webgl_context_loss_and_restore_handlers() -> None:
    ...
    content = HERO_SCENE_JS.read_text(encoding="utf-8")
    assert (
        "webglcontextlost" in content
    ), "webglcontextlost listener not found in hero-scene.js"
    assert (
        "webglcontextrestored" in content
    ), "webglcontextrestored listener not found in hero-scene.js"
    assert (
        "preventDefault" in content
    ), "preventDefault() on webglcontextlost not found in hero-scene.js"
...
def test_hero_scene_dispose_hero_node_export_and_teardown() -> None:
    ...
    content = HERO_SCENE_JS.read_text(encoding="utf-8")
    assert (
        "_disposeHeroNode" in content
    ), "window._disposeHeroNode teardown hook not found in hero-scene.js"
    assert (
        "cancelAnimationFrame" in content
    ), "cancelAnimationFrame not found in hero-scene.js"
...
def test_hero_scene_cold_luxury_palette_tokens() -> None:
    ...
    has_cobalt = any(
        c in content
        for c in ["3b82f6", "2979ff", "59, 130, 246", "0x3b82f6", "--accent-cobalt"]
    )
    has_cyan = any(
        c in content
        for c in ["06b6d4", "00e5ff", "6, 182, 212", "0x06b6d4", "--accent-cyan"]
    )
...
def test_hero_scene_node_vm_lifecycle_and_disposal() -> None:
    ...
    assert (
        result.returncode == 0
    ), f"Node VM lifecycle test failed:\nSTDOUT:\n{result.stdout}\nSTDERR:\n{result.stderr}"
