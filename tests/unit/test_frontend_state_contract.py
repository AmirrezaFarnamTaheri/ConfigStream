...
def test_main_and_proxies_implement_5_state_trust_contract() -> None:
    ...
    for controller_name, code in [("main.js", main_js), ("proxies.js", proxies_js)]:
        ...
        assert "applyTrustState" in code or "handleTrustState" in code, (
        ...
        assert "error" in code or "empty" in code, f"{controller_name} missing error/empty state handling"
...
def test_freshness_timestamps_strictly_derive_from_metadata_last_updated() -> None:
    ...
    assert "last_updated_utc" in main_js, "main.js must derive freshness from metadata.last_updated_utc"
    assert "last_updated_utc" in proxies_js, "proxies.js must derive freshness from metadata.last_updated_utc"
    ...
    assert not footer_date_spoof_pattern.search(main_js), "main.js must not spoof footer update using visitor clock"
    assert not footer_date_spoof_pattern.search(proxies_js), "proxies.js must not spoof footer update using visitor clock"
...
def test_invalid_security_state_blocks_operational_actions() -> None:
    ...
    assert "canDistribute" in proxies_js or "isOperationalBlocked" in proxies_js or "isActionAllowed" in proxies_js, (
    ...
    assert "Detached cryptographic verification failed" in main_js or "Detached cryptographic verification failed" in proxies_js or "Security Alert" in main_js or "Security Alert" in proxies_js
...
def test_error_and_empty_states_provide_actionable_retry() -> None:
    ...
    assert "retry" in proxies_js.lower(), "proxies.js must provide actionable retry trigger on error"
    assert "retry" in main_js.lower(), "main.js must provide retry or reconnect mechanism"
