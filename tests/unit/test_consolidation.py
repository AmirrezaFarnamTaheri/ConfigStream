...
def test_calculate_compound_score():
    ...
    p_stale = Proxy(config="test", protocol="vmess", address="1.1.1.1", port=443, latency=100, stale=True)
    assert calculate_compound_score(p_stale) == 150.0 # 100 * 1.5

    p_none = Proxy(config="test", protocol="vmess", address="1.1.1.1", port=443, latency=None)
...
def test_select_top_configs(sample_proxies):
    ...
    selected_all = select_top_configs(sample_proxies, top_per_protocol=1, total_limit=100)
    assert len(selected_all) == 4

    # Verify uniqueness logic
    dupe_proxies = sample_proxies + [sample_proxies[0]] # Add duplicate
    selected_dupe = select_top_configs(dupe_proxies, top_per_protocol=10, total_limit=100)
