...
async def test_singbox_tester_dry_run(mock_settings):
    ...
    proxy = Proxy(config="test", protocol="vmess", address="1.1.1.1", port=443, uuid="uuid")
...
async def test_singbox_tester_batch_dry_run(mock_settings):
    tester = SingBoxTester(dry_run=True)
    proxies = [
        Proxy(config="test1", protocol="vmess", address="1.1.1.1", port=443, uuid="uuid1"),
        Proxy(config="test2", protocol="vless", address="2.2.2.2", port=443, uuid="uuid2")
    ]
...
async def test_singbox_tester_cache_hit(mock_settings):
    cache = MagicMock()
    cached_proxy = Proxy(config="test", protocol="vmess", address="1.1.1.1", port=443, uuid="uuid", is_working=True, latency=50)
...
async def test_singbox_tester_python_direct(mock_settings):
    ...
    tester.python_tester.test_direct = AsyncMock(return_value=MagicMock(is_working=True))

    proxy = Proxy(config="http://user:pass@1.1.1.1:8080", protocol="http", address="1.1.1.1", port=8080)
...
async def test_singbox_tester_go_fallback(mock_settings):
    ...
    tester.python_tester.test_via_singbox = AsyncMock(return_value=MagicMock(is_working=True))

    proxies = [Proxy(config="test", protocol="vmess", address="1.1.1.1", port=443, uuid="uuid")]
