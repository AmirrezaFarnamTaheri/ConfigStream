...
async def test_failure_mode_cooldown(tmp_path, monkeypatch):
    ...
    src_file.write_text("https://example.com/sub", encoding="utf-8")
    ...
    # FIX: Make it async
    async def fake_update(): return None
...
async def test_failure_mode_anomaly_db_crash(tmp_path, monkeypatch):
    ...
    """
    src_file = tmp_path / "sources.txt"
    src_file.write_text("ss://YWVzLTI1Ni1nY206cGFzc3dvcmRAMC4wLjAuMDo4Mzgy#Test", encoding="utf-8")
    ...
    monkeypatch.setattr("configstream.source_quality.SourceQualityTracker.should_fetch", lambda s, u: True)

    # Mock network fetch
    from configstream.fetcher_core.models import FetchResult

    async def fake_fetch(*args, **kwargs):
        return {"https://example.com/sub": FetchResult(url="https://example.com/sub", content="ss://YWVzLTI1Ni1nY206cGFzc3dvcmRAMC4wLjAuMDo4Mzgy#Test", status=200)}

    monkeypatch.setattr("configstream.fetcher_core.batch.fetch_multiple_sources", fake_fetch)
    ...
    monkeypatch.setattr("configstream.geoip.GeoIPResolver.lookup", lambda s, ip: GeoData("US", "United States", "City"))
