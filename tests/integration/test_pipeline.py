...
async def test_run_full_pipeline_remote_source(mocker, tmp_path, no_pool_shutdown):
    ...
    mocker.patch(
        ...
        new_callable=AsyncMock,
        return_value={
            "http://remote.com/source": FetchResult(
                source="http://remote.com/source", success=True, content=config
            )
        },
    )
    mocker.patch(
...
async def test_run_full_pipeline_remote_source_failure(mocker, tmp_path, caplog, no_pool_shutdown):
    ...
    mocker.patch(
        ...
        new_callable=AsyncMock,
        return_value={
            "http://failing-remote.com/source": FetchResult(
                source="http://failing-remote.com/source",
                success=False,
                error="timeout",
                content="",
            )
