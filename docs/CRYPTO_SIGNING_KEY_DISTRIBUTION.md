...
- **Explicit unsigned policy**: `config/pages-trust-policy.json` may opt one exact repository into genuinely unsigned Pages artifacts, and its repository binding prevents inherited fork activation. Repository Variable `ALLOW_UNSIGNED_PAGES=true|false` overrides the committed default. The same resolved policy is required for candidate verification, deployed smoke, and rollback smoke. It never authorizes a signed artifact whose public trust anchor is missing or invalid.
- **Rollback capture is deliberately exempt from the publication policy**: the currently served release is always snapshot-able, even when it is unsigned, because a strict policy that cannot capture the site it is replacing deadlocks its own cutover (no rollback baseline, therefore no deploy, therefore the fix can never be published). `scripts/snapshot_pages_release.py` and `scripts/validate_pages_signature_policy.py` therefore honour the explicit opt-in for the unsigned case whether or not a trust anchor is configured, while still refusing to downgrade a signed artifact and still rejecting an unsigned candidate when no opt-in is given. `tests/unit/test_pages_deploy_workflow.py` pins this in the workflow.
